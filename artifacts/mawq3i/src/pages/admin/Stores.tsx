...
import { Plus, LogIn, Eye, ExternalLink, Pencil, BanIcon, CheckCircle2 } from 'lucide-react';
...
export default function AdminStores() {
  ...
  const [stores, setStores] = useState<StoreRecord[]>(adminStores);
  ...
  const handleAdd = () => {
    ...
    const store: StoreRecord = {
      id: String(Date.now()), name: newStore.name, slug: newStore.domain.split('.')[0],
      domain: newStore.domain, ownerName: newStore.ownerName, ownerEmail: newStore.ownerEmail,
      ownerPhone: newStore.ownerPhone, currency: newStore.currency as 'ILS' | 'SAR',
      status: 'active', ordersCount: 0, totalSales: 0,
      subscriptionStatus: 'trial', subscriptionPlan: 'monthly', subscriptionPaid: false,
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="bg-[#0e1217] border-white/10 sm:max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAdd}>{isAr ? 'إضافة' : 'Add'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
