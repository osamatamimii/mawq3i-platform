...
export async function getBlogPostsForStore(storeId: string, useAdmin = false): Promise<BlogPost[]> {
  try {
    if (useAdmin) {
      const rows = await adminRest.select('blog_posts', `store_id=eq.${storeId}&order=created_at.desc`);
      return rows.map(rowToBlogPost);
    }
    const { data, error } = await supabase.from('blog_posts').select('*').eq('store_id', storeId).order('created_at', { ascending: false });
    if (error || !data) return [];
...
export async function getBlogPost(id: string, useAdmin = false): Promise<BlogPost | null> {
  try {
    if (useAdmin) {
      const rows = await adminRest.select('blog_posts', `id=eq.${id}`);
...
export async function createBlogPost(storeId: string, post: {
  title: string; slug: string; excerpt: string; content: string; coverImageUrl: string;
  metaTitle: string; metaDescription: string; status: 'draft' | 'published'; authorName: string;
}, useAdmin = false): Promise<BlogPost | null> {
  try {
    const row = {
      store_id: storeId,
      title: post.title,
    ...
    if (useAdmin) {
      const res = await adminRest.insert('blog_posts', row);
      return res ? rowToBlogPost(res) : null;
    }
...
export async function updateBlogPost(id: string, updates: {
  ...
  try {
    ...
    if (updates.metaTitle !== undefined) row.meta_title = updates.metaTitle || null;
    if (updates.metaDescription !== undefined) row.meta_description = updates.metaDescription || null;
    if (updates.authorName !== undefined) row.author_name = updates.authorName || null;
    if (updates.status !== undefined) {
      row.status = updates.status;
      if (updates.status === 'published' && !updates.wasPublished) {
    ...
    if (useAdmin) {
      return await adminRest.update('blog_posts', `id=eq.${id}`, row);
    }
...
export async function deleteBlogPost(id: string, useAdmin = false): Promise<boolean> {
  try {
    if (useAdmin) {
      return await adminRest.delete('blog_posts', `id=eq.${id}`);
    }
